 ...
 */
package org.traccar.client.oba.io;
...
public class ObaContext {
    ...
    public void setBaseTraccarUrl(Context context, Uri.Builder builder) {
        // Use the custom Traccar url if vailable
        String traccarBaseUrl = Application.get().getCustomTraccarUrl();
        if (TextUtils.isEmpty(traccarBaseUrl)) {
            // Use the current region OTP base URL
            traccarBaseUrl = Application.get().getCurrentRegion().getTraccarBaseUrl();
            Log.d(TAG, "Using default region Traccar URL '" + traccarBaseUrl + "'.");
        } else {
            Log.d(TAG, "Using custom Traccar URL set by user '" + traccarBaseUrl + "'.");
        }
        setUrl(context, builder, traccarBaseUrl);
    }
